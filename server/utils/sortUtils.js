...
export const vendorShadingState = (vendor, existsInWholeSet, existsInFilteredSet, vendorFilterState, isStatusFilterCall) => {
  let shadingState = '';
  if (existsInWholeSet && existsInFilteredSet) {
    shadingState = 'ON';
  } else if (existsInWholeSet && !existsInFilteredSet && isStatusFilterCall) {
    shadingState = 'GREYED';
...
export const statusShadingState = (stateName, existsInSet, statusFilterFromFrontEnd) => {
  let shadingState = '';
  if (existsInSet && statusFilterFromFrontEnd === 'FILTERED') {
    shadingState = 'FILTERED';
